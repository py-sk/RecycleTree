...
export default class UploadBar extends Component {
  ...
  imageAPICall(image) {
    ...
    axios
      .post("https://recycletree.herokuapp.com/predictFake", formData)
      .then((res) => {
      ...
      return (
        <div>
          ...
          <DropzoneDialog
            ...
            filesLimit={1}
          />
          <ResultsPanel mat={this.state.preds[0]} item = {this.state.preds[1]} yes={this.state.preds[2]} no={this.state.preds[3]} preview = {this.state.preview} />
        </div>
      );
    } else {
      return (
        <div>
          <ResultsPanel mat={this.state.preds[0]} item = {this.state.preds[1]} yes={this.state.preds[2]} no={this.state.preds[3]} preview = {this.state.preview} />
          <br/>
          <Button color="secondary" variant="contained" onClick={this.handleReset.bind(this)}>Upload New Image</Button>
        </div>
      );
    }
  }
